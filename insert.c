...
int read_and_validate(int argc, char *argv[], slist **head){
    if(argc == 1){
        printf("[FAILURE] No input files provided.\n");
        printf("[INFO] Usage : ./a.out <file1.txt> <file2.txt> ...\n");
        return FAILURE;
    }
    ...
    for (int i = 1; i < argc; i++){
        ...
        if (extn == NULL || strcmp(extn, ".txt") != 0){
            printf("This file '%s' — not a .txt file.\n", argv[i]);
            continue;
        }
        ...
        if (fptr == NULL){
            printf("This file '%s' — unable to open.\n", argv[i]);
            continue;
        }
        ...
        if (search_file(*head, argv[i]) == SUCCESS){
            printf("Repeated '%s' — duplicate file.\n", argv[i]);
            fclose(fptr);
            continue;
        }
